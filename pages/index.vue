...
const pickupStreet = ref<string>("");
const pickupNumber = ref<string>("");
const pickupZip = ref<string>("");
...
const today = computed(() => {
  const d = new Date();
  // Format to YYYY-MM-DD which is required for the 'min' attribute of a date input
...
const pickupAddressForMap = computed(() => {
  if (pickupStreet.value && pickupCity.value && pickupZip.value) {
    const address = `${pickupStreet.value} ${pickupNumber.value || ''}, ${pickupZip.value} ${pickupCity.value}, Italy`;
...
const nextStep = () => {
  if (currentStep.value < totalSteps) {
    ...
    if (currentStep.value === 2 && !selectedService.value) {
      alert("Seleziona un tipo di intervento.");
      return;
    }
    if (currentStep.value === 3 && selectedFiles.value.length === 0) {
      alert("Carica almeno una foto.");
      return;
    }
    if (currentStep.value === 4 && !deliveryMethod.value) {
        alert("Scegli un metodo di spedizione.");
        return;
    }
    currentStep.value++;
  }
...
const placeholders: Record<string, string[]> = {
  scarpe: ["Riparazione suola rotta destra", "Tingere parte anteriore", "Fare la punta tonda della scarpa", "Allargare la tomaia"],
  borse: ["Sostituire cerniera", "Riparare manico", "Tingere angoli rovinati", "Sostituzione ruote trolley"],
  abbigliamento: ["Cucire strappo sulla manica", "Sostituire fodera interna", "Rammendare buco su un maglione"],
  accessori: ["Accorciare cintura in pelle", "Sostituire fibbia rotta", "Cucire un portafoglio scucito"]
};

const animatedPlaceholder = ref("Dettagli extra...");
let placeholderInterval: any;

const startPlaceholderAnimation = (category: string) => {
  if (placeholderInterval) clearInterval(placeholderInterval);
  const categoryPlaceholders = placeholders[category as keyof typeof placeholders] || [];
  if (categoryPlaceholders.length > 0) {
    let currentIndex = 0;
    animatedPlaceholder.value = categoryPlaceholders[currentIndex] || "";
    placeholderInterval = setInterval(() => {
      currentIndex = (currentIndex + 1) % categoryPlaceholders.length;
      animatedPlaceholder.value = categoryPlaceholders[currentIndex] || "";
    }, 2500);
  } else {
    animatedPlaceholder.value = "Dettagli extra...";
  }
};

watch(selectedCategory, (newCategory) => {
  if (newCategory) {
    startPlaceholderAnimation(newCategory);
  } else {
    if (placeholderInterval) clearInterval(placeholderInterval);
    animatedPlaceholder.value = "Dettagli extra...";
  }
...
const startQuoteRequest = async () => {
    if(!selectedCategory.value || !selectedService.value || selectedFiles.value.length === 0 || !deliveryMethod.value) {
        alert("Per favore compila tutti i campi: foto e metodo di consegna.");
        return;
    }

    if (deliveryMethod.value === 'Ritiro a Domicilio') {
        if (!pickupCity.value || !pickupStreet.value || !pickupNumber.value || !pickupZip.value || !pickupDate.value) {
            alert("Per il ritiro a domicilio, specifica l'indirizzo completo (Città, Via, Civico, CAP) e la data.");
            return;
        }
        if (new Date(pickupDate.value) < new Date(today.value)) {
            alert("La data di ritiro non può essere nel passato.");
            return;
        }
    }
    ...
    try {
        ...
        // 2. Add "Preventivo" Product to Cart & Redirect
        const preventivoProduct = preventivoData.value?.product;

        if (preventivoProduct) {
            // 3. Prepare metadata to attach to the order
            const fullAddress = `${pickupStreet.value} ${pickupNumber.value}, ${pickupZip.value} ${pickupCity.value}`;

            const metaData = [
                { key: 'Richiesta Riparazione', value: 'Sì' },
                ...
                { key: 'Metodo Consegna', value: deliveryMethod.value },
                { key: 'Note Cliente', value: repairNotes.value || 'Nessuna' },
                ...uploadedImageUrls.map((url, i) => ({ key: `Foto ${i + 1}`, value: url }))
            ];

            if (deliveryMethod.value === 'Ritiro a Domicilio') {
                metaData.push({ key: 'Indirizzo Ritiro', value: fullAddress });
                metaData.push({ key: 'Data Ritiro', value: pickupDate.value });
            }

            // 4. Prepare Customer Note (URLs are clickable in Admin Notes)
            let noteContent = `RICHIESTA RIPARAZIONE\n`;
            noteContent += `Categoria: ${repairOptions[selectedCategory.value].label}\n`;
            noteContent += `Servizio: ${selectedService.value}\n`;
            noteContent += `Marca: ${repairBrand.value || 'N/A'}\n`;
            noteContent += `Metodo Consegna: ${deliveryMethod.value}\n`;
            if (deliveryMethod.value === 'Ritiro a Domicilio') {
                noteContent += `Indirizzo Ritiro: ${fullAddress}\n`;
                noteContent += `Data Ritiro: ${pickupDate.value}\n`;
            }
            noteContent += `Note: ${repairNotes.value || 'Nessuna'}\n\n`;
            noteContent += `FOTO:\n` + uploadedImageUrls.map((url, i) => `${i + 1}. ${url}`).join('\n');

            // 5. Attach metadata and note to the order
            orderInput.value.metaData = [
                { key: 'order_via', value: 'WooNuxt Repair Wizard' },
                ...metaData
            ];

            // 6. Pre-fill customer note
            if (!orderInput.value.billing) {
                orderInput.value.billing = {};
            }

            // Pre-fill Billing/Shipping address if Pickup is selected
            if (deliveryMethod.value === 'Ritiro a Domicilio') {
                orderInput.value.billing.address1 = `${pickupStreet.value}, ${pickupNumber.value}`;
                orderInput.value.billing.city = pickupCity.value;
                orderInput.value.billing.postcode = pickupZip.value;
                orderInput.value.billing.country = 'IT'; // Default to Italy
                
                // Copy to shipping as well
                orderInput.value.shipping = { ...orderInput.value.billing };
            }
            
            orderInput.value.customerNote = noteContent;

            // 7. Add product to cart and redirect
            await addToCart({ productId: preventivoProduct.databaseId, quantity: 1 });
            router.push('/checkout');
        } else {
            // Fallback if product not found
            console.error("Product with slug 'preventivo' not found in WooCommerce.");
  ...
  <main class="bg-white min-h-screen">
    ...
    <div id="wizard-section" class="container mx-auto px-4 -mt-10 mb-16 relative z-10">
      ...
      <div v-if="isSuccess" class="bg-white rounded-2xl shadow-xl p-12 text-center max-w-3xl mx-auto border border-green-100 animate-fade-in-up">
        ...
        <button @click="isSuccess = false; selectedCategory = null; selectedFiles = []; repairNotes = ''; deliveryMethod = ''; pickupCity = ''; pickupStreet = ''; pickupNumber = ''; pickupZip = ''; pickupDate = ''" class="text-amber-700 font-semibold hover:underline">
      ...
      <div v-else class="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-5xl mx-auto ring-1 ring-gray-900/5">
        ...
        <div class="w-full bg-gray-100 h-2.5">
            <div class="bg-amber-700 h-2.5 transition-all duration-300" :style="{ width: `${((currentStep - 1) / (totalSteps - 1)) * 100}%` }"></div>
        </div>
        ...
        <div class="p-6 md:p-10">
            ...
            <div v-if="currentStep === 3">
                ...
                <div v-if="selectedCategory === 'scarpe'" class="text-sm text-gray-500 flex items-start gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200 mb-4">
                    <Icon name="ion:information-circle-outline" class="flex-shrink-0 mt-0.5 text-gray-400" size="18" />
                    <p>Per un risultato ottimale, carica anche una foto della scarpa sana. Ci aiuterà a replicare materiali e colori alla perfezione.</p>
                </div>
            ...
            <div v-if="currentStep === 4">
                ...
                <label class="block text-sm font-medium text-gray-700 mb-2">Come invierai il prodotto?</label>
                <select v-model="deliveryMethod" class="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-800 focus:border-amber-800 outline-none">
                    <option value="" disabled selected>Seleziona opzione...</option>
                    <option value="Ritiro a Domicilio">Ritiro a Domicilio (Corriere)</option>
                    <option value="Spedizione a cura del cliente">Spedisco io in laboratorio</option>
                </select>

                <div v-if="deliveryMethod === 'Ritiro a Domicilio'" class="mt-4 space-y-4 animate-fade-in-up p-4 bg-gray-50 border border-gray-200 rounded-xl">
                    <p class="text-sm font-medium text-gray-700">Indirizzo di Ritiro</p>
                    <div class="grid grid-cols-2 gap-3 mb-3">
                        <input type="text" v-model="pickupCity" placeholder="Città" class="w-full p-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-800 outline-none" />
                        <input type="text" v-model="pickupZip" placeholder="CAP" class="w-full p-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-800 outline-none" />
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                        <div class="col-span-2">
                            <input type="text" v-model="pickupStreet" placeholder="Via / Piazza" class="w-full p-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-800 outline-none" />
                        </div>
                        <div>
                            <input type="text" v-model="pickupNumber" placeholder="N. Civico" class="w-full p-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-800 outline-none" />
                        </div>
                    </div>
                    <a v-if="pickupAddressForMap" :href="pickupAddressForMap" target="_blank" class="flex items-center gap-1 !mt-2 text-amber-700 font-bold hover:underline text-sm">
                        <Icon name="mdi:map-marker" /> Controlla indirizzo su Google Maps
                    </a>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Quando? (Data preferita)</label>
                    <input type="date" v-model="pickupDate" :min="today" class="w-full p-3 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-amber-800 outline-none" />
                </div>

                <div v-if="deliveryMethod === 'Spedizione a cura del cliente'" class="mt-4 p-4 bg-amber-50 border border-amber-100 rounded-xl text-sm text-amber-900 animate-fade-in-up">
                    <p class="font-bold mb-1">Spedisci il tuo pacco a:</p>
                    <p>Calzolaio Manu<br>Via Riccione, 13<br>70022 Altamura BA</p>
                    <a href="https://maps.app.goo.gl/hA12DVyD8wpwWcM78" target="_blank" class="flex items-center gap-1 mt-3 text-amber-700 font-bold hover:underline">
                        <Icon name="mdi:map-marker" /> Vedi su Google Maps
                    </a>
                </div>
            </div>
            
            <!-- Step 5: Notes & Submit -->
            <div v-if="currentStep === 5" class="grid md:grid-cols-2 gap-8">
                <div>
                    <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                        <span class="bg-amber-800 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm mr-3 font-bold">5</span>
                        Dettagli Finali
                    </h3>
                 ...
                 <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2 mt-11">Marca (Opzionale)</label>
            ...
            <div class="mt-10 p-4 bg-amber-50 border border-amber-100 rounded-lg flex items-center gap-3 text-amber-900">
                <Icon name="mdi:truck-check-outline" size="24" />
                <p class="text-sm font-medium">Ritiro e riconsegna a domicilio inclusi. Pensiamo a tutto noi.</p>
            </div>
            ...
            <div class="mt-6 pt-6 border-t border-gray-100 flex justify-between items-center">
                <button 
                    @click="prevStep" 
                    v-if="currentStep > 1"
                    class="text-gray-600 font-semibold hover:text-black transition-all flex items-center gap-2"
                >
                ...
                <button 
                    ...
                    class="bg-amber-800 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-amber-900 transition-all flex items-center gap-2"
                >
                    Avanti
                    <Icon name="ion:arrow-forward-outline" />
                </button>
                ...
                <button 
                    ...
                    class="bg-black text-white text-lg font-bold py-4 px-10 rounded-xl shadow-xl hover:bg-gray-800 transition-all disabled:opacity-50 flex items-center gap-3"
                >
                    <span v-if="!isSubmitting">Richiedi preventivo</span>
