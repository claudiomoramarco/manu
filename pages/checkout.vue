...
const isPreventivo = computed(() =>
  cart.value?.contents?.nodes?.some((item: any) => item.product?.node?.slug === 'preventivo' || item.product?.slug === 'preventivo'),
);

const buttonText = computed(() => {
  if (isProcessingOrder.value || isStripeProcessing.value) return t('general.processing');

  if (isPreventivo.value) return "Richiedi Preventivo";

  return t('shop.checkoutButton');
});
const isStripeElementReady = ref<boolean>(false);
const stripeClientSecret = ref<string>('');

const preventivoDetails = computed(() => {
  if (!isPreventivo.value) return null;

  const meta = orderInput.value.metaData;
  if (!meta) return null;

  const findMeta = (key: string) => meta.find((m: { key: string; value: string }) => m.key === key)?.value;

  return {
    categoria: findMeta('Categoria'),
    servizio: findMeta('Servizio'),
    marca: findMeta('Marca'),
    metodoConsegna: findMeta('Metodo Consegna'),
    note: findMeta('Note Cliente'),
    indirizzoRitiro: findMeta('Indirizzo Ritiro'),
    dataRitiro: findMeta('Data Ritiro'),
    foto: meta.filter((m: { key: string; value: string }) => m.key.startsWith('Foto')).map((m: { key: string; value: string }) => ({ key: m.key, url: m.value }))
  };
});
  ...
  <div class="flex flex-col min-h-150">
    <template v-if="cart && customer">
      ...
      <form v-else class="container flex flex-wrap items-start gap-8 my-16 justify-evenly lg:gap-20" @submit.prevent="payNow">
        ...
        <template v-if="isPreventivo && preventivoDetails">
          <div class="w-full md:w-2/5 lg:w-1/3">
            <div class="p-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow-sm border dark:border-gray-700/50 sticky top-24">
              <h3 class="text-xl font-bold mb-6 text-gray-900 dark:text-white">Riepilogo Preventivo</h3>
              
              <div class="space-y-4 text-sm">
                <div class="flex justify-between">
                  <span class="text-gray-500 dark:text-gray-400">Categoria:</span>
                  <span class="font-semibold text-right">{{ preventivoDetails.categoria }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-500 dark:text-gray-400">Servizio:</span>
                  <span class="font-semibold text-right">{{ preventivoDetails.servizio }}</span>
                </div>
                <div v-if="preventivoDetails.marca && preventivoDetails.marca !== 'N/A'" class="flex justify-between">
                  <span class="text-gray-500 dark:text-gray-400">Marca:</span>
                  <span class="font-semibold text-right">{{ preventivoDetails.marca }}</span>
                </div>
                <hr class="dark:border-gray-700 !my-4">
                <div class="flex justify-between">
                  <span class="text-gray-500 dark:text-gray-400">Consegna:</span>
                  <span class="font-semibold text-right">{{ preventivoDetails.metodoConsegna }}</span>
                </div>
                <div v-if="preventivoDetails.metodoConsegna === 'Ritiro a Domicilio'" class="text-right text-xs text-gray-500 dark:text-gray-400 -mt-2">
                  <p>{{ preventivoDetails.indirizzoRitiro }}</p>
                  <p>il {{ preventivoDetails.dataRitiro }}</p>
                </div>
                <hr class="dark:border-gray-700 !my-4">
                <div>
                  <span class="text-gray-500 dark:text-gray-400 block mb-2">Foto Caricate:</span>
                  <div class="grid grid-cols-3 gap-2">
                    <div v-for="foto in preventivoDetails.foto" :key="foto.key" class="aspect-square rounded-md overflow-hidden bg-gray-100 dark:bg-gray-700 border dark:border-gray-700">
                      <img :src="foto.url" class="w-full h-full object-cover" alt="Foto riparazione">
                    </div>
                  </div>
                </div>
                <div v-if="preventivoDetails.note && preventivoDetails.note !== 'Nessuna'">
                  <span class="text-gray-500 dark:text-gray-400 block mb-1">Le tue note:</span>
                  <p class="text-xs p-3 bg-gray-100 dark:bg-gray-900/50 rounded-md italic">"{{ preventivoDetails.note }}"</p>
                </div>
              </div>
              <Button :loading="isProcessingOrder || isStripeProcessing" :disabled="isCheckoutDisabled" size="lg" type="submit" class="w-full mt-8">
                {{ buttonText }}
              </Button>
            </div>
          </div>
        ...
        <OrderSummary v-else>
          <Button :loading="isProcessingOrder || isStripeProcessing" :disabled="isCheckoutDisabled" size="lg" type="submit" class="w-full mt-4">
            {{ buttonText }}
          </Button>
        </OrderSummary>
      </form>
